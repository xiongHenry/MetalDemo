//
//  DynamicSplitScreenViewController.swift
//  Metal-10(视频播放)
//
//  Created by 熊涛 on 2019/12/23.
...
class DynamicSplitScreenViewController: UIViewController, XTVideoMovieDelegate {
    ...
    func perpare(at currentTime: CMTime) {
        let i = (CMTimeGetSeconds(currentTime) / CMTimeGetSeconds(asset.duration)) * 4
        if Int(i) > count {
            renderView.addTextureCount()
            count += 1
        }
    }
    ...
    // MARK: - lazy
    lazy var renderView: XTRenderView2 = {
        let v = XTRenderView2(frame: self.view.bounds)
        return v
    }()
