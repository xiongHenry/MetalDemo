...
class XTMetalView: UIView {
    ...
    func render() {
        ...
        ///1.顶点数据
        let vertices = [
            XTVertex(position: [0.5, -0.5], color: [1,0,0,1]),
            XTVertex(position: [-0.5, -0.5], color: [0,1,0,1]),
            XTVertex(position: [0.0, 0.5], color: [0,0,1,1])
        ]
        let buffer = device?.makeBuffer(bytes: vertices, length: MemoryLayout<XTVertex>.size * 3, options: .cpuCacheModeWriteCombined)
