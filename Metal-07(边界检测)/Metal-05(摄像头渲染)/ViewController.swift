...
class ViewController: UIViewController, AVCaptureVideoDataOutputSampleBufferDelegate,MTKViewDelegate {
    ...
    func draw(in view: MTKView) {
        //把MTKView作为目标纹理
        guard let _texture = texture, let _destTexture = destTexture, let drawingTexture = view.currentDrawable?.texture else {
            return
        }
        ...
        computeEncoder?.setTexture(_destTexture, index: 1)
        /// 设置计算区域
        computeEncoder?.dispatchThreadgroups(groupCount, threadsPerThreadgroup: groupSize)
        /// 调用endEncoding释放编码器，下一个encode才能创建
        ...
        ///设置纹理数据
        renderCommandEncoder?.setFragmentTexture(_destTexture, index: 0)
    ...
    func getPipelineState() {
        ...
        let kernelFunction = library.makeFunction(name: "sobelKernel")
